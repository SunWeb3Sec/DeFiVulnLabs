...
function setUp() public { 
        ...
        SimplePoolContract = new SimplePool(address(USDaContract),address(USDbContract));
        SimpleBankContract = new SimpleBank(address(USDaContract),address(SimplePoolContract),address(USDbContract));
...
function testPrice_Manipulation() public {
    ...
    console.log("There are 1000 USDa and USDb in the pool, so the price of USDa is 1 to 1 USDb.");
    emit log_named_decimal_uint("Current USDa convert rate",SimplePoolContract.getPrice(),18);
    ...
    fallback() external { //flashlon callback
    
    emit log_named_decimal_uint("Price manupulated, USDa convert rate",SimplePoolContract.getPrice(),18);  // 1 USDa : 2 USDb
    ...
    emit log_named_decimal_uint("Use 100 USDa to convert, My USDb balance",USDbContract.balanceOf(address(this)),18);
...
contract SimplePool {
    ...
    function getPrice() public view returns(uint256) {
        ...
        uint256 USDaAmount = USDa.balanceOf(address(this));
        uint256 USDbAmount = USDb.balanceOf(address(this));
...
contract SimpleBank {
    ...
    function exchange(uint256 amount) public {
        ...
        require(payoutToken.transfer(msg.sender, tokensToReceive), "Payout transfer failed");
    }
}
